...
### Live Detection
- **Attack Scenarios** — 5 кнопок для запуска конкретных типов атак:
  | Сценарий | Цвет | Описание |
  |---|---|---|
  | Port Scan | жёлтый | Систематический перебор открытых портов |
  | DDoS | красный | Распределённая атака отказа в обслуживании |
  | DoS Hulk | оранжевый | HTTP GET flood через случайные URL |
  | Botnet | фиолетовый | Трафик C&C ботнета |
  | DoS GoldenEye | голубой | HTTP keepalive DoS атака |
- Каждый сценарий загружает **реальные строки из CICIDS2017** соответствующего класса и прогоняет их через ансамбль ML — это не заглушка, а настоящий инференс на реальных паттернах атак
- Строки появляются **одна за другой** с анимацией (120 мс задержка) — визуальный эффект "живого сканирования"
- Прогресс-бар `N / 25 flows analysed` с цветом под тип атаки
- Баннер `⚠ N INTRUSIONS DETECTED` с процентом detection rate
- Таблица: Label, Prediction, Confidence (bar), голоса IF/LOF/SVM/RF, Score
- Каждому сценарию назначены реальные публичные IP атакующих (из известных сканеров/ботнетов)
- Запуск случайной симуляции на 100 строках по-прежнему доступен кнопкой "Run Random Simulation"
...
| `POST` | `/api/simulate/attack/{type}` | Сценарий конкретной атаки (DDoS, PortScan, DoS Hulk, Bot, DoS GoldenEye) — реальные строки из датасета через ансамбль |
